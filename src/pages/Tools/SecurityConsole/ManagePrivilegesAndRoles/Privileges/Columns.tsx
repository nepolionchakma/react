import { IPrivilege, Users } from "@/types/interfaces/users.interface";
import { convertDate } from "@/Utility/DateConverter";
import { renderUserName } from "@/Utility/NotificationUtils";
import { ColumnDef } from "@tanstack/react-table";
import { ArrowUpDown } from "lucide-react";

export const getColumns = (users: Users[]): ColumnDef<IPrivilege>[] => [
  {
    id: "select",
    size: 24,
    minSize: 24,
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex justify-center min-w-max py-1">
        {row.getValue("privilege_id")}
      </div>
    ),
  },
  {
    accessorKey: "privilege_name",
    enableResizing: true,
    sortingFn: (rowA, rowB, columnId) => {
