...
const Modal = ({
  ...
  const form = useForm<jobTitleForm>({
    ...
    defaultValues: {
      job_title_name: action === "edit" ? selectedItems[0].job_title_name : "",
    },
  });
    ...
    if (action === "edit" && selectedItems[0]) {
      const fetchRole = async () => {
        const res = await loadData({
          baseURL: FLASK_URL,
          url: `${flaskApi.JobTitles}?job_title_id=${selectedItems[0].job_title_id}`,
  ...
  const onSubmit = async (data: jobTitleForm) => {
    if (action === "add") {
      const payload = {
        ...
        tenant_id: combinedUser?.tenant_id,
      };
