...
const RegisterLookups = () => {
  ...
  const [selectedLookups, setSelectedLookups] = useState<ILookup[]>([]);
  ...
  useEffect(() => {
    const params = {
      baseURL: FLASK_URL,
      url: `/lookup_with_values?tenant_id=${enterpriseSetting?.tenant_id}&lookup_code=${query}&page=${currentPage}&limit=${limit}`,
  ...
  useEffect(() => {
    if (data?.length > 0) {
      if (selectedLookups.length !== data.length) {
        setIsSelectAll(false);
      } else {
        setIsSelectAll(true);
      }
    }
    const ids = selectedLookups?.map((item) => item.lookup_id);
    setSelectedIds(ids);
  }, [data?.length, selectedLookups]);

  const handleRowSelection = (rowData: ILookup) => {
    setSelectedLookups((prev) => {
      const webhook = prev.find((item) => item.lookup_id === rowData.lookup_id);

      if (webhook) {
        const filtered = prev.filter(
          (item) => item.lookup_id !== rowData.lookup_id,
        );
        return filtered;
      } else {
        return [rowData, ...prev];
    ...
    <>
        ...
        <div className="flex items-center gap-2">
          <ActionButtons>
            ...
            {grantedPrivlegeIds?.includes(11103) && (
              <button disabled={selectedLookups.length !== 1}>
                <CustomTooltip tooltipTitle="Edit">
                  <FileEdit
                    className={`${
                      selectedLookups.length !== 1
            ...
            {grantedPrivlegeIds?.includes(11104) && (
              <Alert
                disabled={selectedLookups.length === 0 || isDeleteLoading}
                actionName="delete"
                ...
                <>
                  {isDeleteLoading ? (
                    ...
                    <span className="flex flex-col items-start">
                      {selectedLookups.map((item, index) => (
                        <span key={item.lookup_name}>
                          {index + 1}. Lookup Name : {item.lookup_name}
                        </span>
                      ))}
                    </span>
                  )}
                  ...
                  {row.getVisibleCells().map((cell, index) => (
                    ...
                    >
                      {index === 0 ? (
                        <Checkbox
                          className="mt-1"
                          checked={selectedIds.includes(row.original.lookup_id)}
      ...
      <div className="flex justify-between p-1">
        <div className="flex-1 text-sm text-gray-600">
          {selectedLookups?.length} row(s) selected.
        </div>
        <Pagination5
